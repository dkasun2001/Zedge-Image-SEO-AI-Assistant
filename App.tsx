...
const App: React.FC = () => {
    ...
    const [seoResults, setSeoResults] = useState<Record<string, SeoData | { error: string }>>({});
    ...
    const handleGenerateSeo = useCallback(async (image: ImageData, currentSettings: Settings) => {
        if (!apiKey) {
        ...
        try {
            const result = await generateSeoForImage(image.base64, currentSettings, apiKey);
            setSeoResults(prev => ({ ...prev, [image.id]: result }));
        } catch (error) {
            console.error("Error generating SEO:", error);
            const errorMessage = error instanceof Error ? error.message : "An unknown error occurred.";
            setSeoResults(prev => ({ ...prev, [image.id]: { error: errorMessage } }));
        } finally {
    ...
    const handleDownloadCsv = useCallback(() => {
        ...
        successfulResults.forEach((image) => {
            ...
            const escapedRow = rowData.map(cell => `"${String(cell).replace(/"/g, '""')}"`).join(',');
