            ...
            <div className="space-y-4 relative pr-8">
              <div className="absolute top-0 right-0">
                <button
                  ...
                  className="p-1 hover:bg-red-500/50 rounded text-slate-400 hover:text-white transition-colors"
                >
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {seoData.tags.map((tag) => (
                    <span
                      key={tag}
                      className="px-2 py-1 text-xs font-mono bg-slate-700 text-slate-300 rounded"
                    >
                      {tag}
                    </span>
                  ))}
...
export const ResultsDisplay: React.FC<ResultsDisplayProps> = ({
  ...
  const imagesToProcessCount = images.filter(
    (image) => !seoResults[image.id]
  ).length;
  ...
  const generateAllTitle = !apiKey
    ? "Please set your API key first"
    : isGenerating
    ? "Generation in progress..."
    : imagesToProcessCount === 0
    ? "All images processed"
    : "Generate for all pending images";
  const hasSuccessfulResults = Object.values(seoResults).some(
    (res) => res && !("error" in res)
  );
