  ...
  try {
    ...
    if (
      ...
      return {
        isValid: false,
        error:
          "The provided API key is not valid. Please check it and try again.",
      };
    }
    return {
      isValid: false,
      error:
        "Could not validate the API key. Ensure it is a correct Gemini API key and try again.",
    };
  }
};

export const generateSeoForImage = async (
  base64Image: string,
  settings: Settings,
  apiKey: string
): Promise<SeoData> => {
  ...
  const textPart = {
    text: `Think as top ranked stock photography contributor and analyze this image and generate SEO metadata.
        ...
        - "description": An engaging SEO meta description, maximum ${settings.maxDescriptionLength} characters.
        - "tags": An array of exactly ${settings.tagCount} relevant SEO tags. Each tag must be in snake_case, lowercase and do not repeating same word more than 2 times
